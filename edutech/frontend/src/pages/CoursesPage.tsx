    ...
    <div className="min-h-screen bg-theme-page transition-colors duration-300 pt-20 pb-12 px-4 sm:px-6 lg:px-10 blob-bg">
      <div className="max-w-[1400px] mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex flex-col sm:flex-row justify-between sm:items-end gap-3">
          <div>
            <h1 className="font-['Poppins'] text-[28px] font-extrabold text-theme-text mb-1 tracking-tight">{'\ud83d\udcda'} My Courses</h1>
            <p className="text-sm text-theme-text-muted font-['Poppins']">
            ...
            <motion.div key={i} whileHover={{ y: -4, scale: 1.02 }}
              ...
              <div className={`relative z-[1] w-12 h-12 rounded-2xl ${s.iconBg} flex items-center justify-center mx-auto mb-3`}>
                <span className="text-[26px]">{s.emoji}</span>
                            ...
                            <div className={`relative z-[1] font-['Poppins'] text-[32px] font-extrabold bg-gradient-to-r ${s.gradient} bg-clip-text text-transparent leading-none`}>{s.val}</div>
                            <div className="relative z-[1] text-[11px] font-semibold text-theme-text-muted mt-1.5 uppercase tracking-widest font-['Poppins']">{s.label}</div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex gap-1.5 p-1.5 rounded-[14px] bg-theme-card border border-theme-border">
            {[
              ...
              <button key={f.key} onClick={() => setFilter(f.key)}
                className={`px-[18px] py-2 rounded-[10px] text-[13px] font-bold font-['Poppins'] transition-all ${
                  filter === f.key
          ...
          {hasClassInfo && (
            <button onClick={() => setShowMyClass(!showMyClass)}
              className={`px-3.5 py-2 rounded-[10px] text-[13px] font-bold font-['Poppins'] flex items-center gap-1.5 transition-all border ${
                showMyClass
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            className="w-full sm:w-80 pl-10 pr-4 py-2.5 rounded-xl bg-theme-card border border-theme-border text-sm text-theme-text placeholder-theme-text-muted focus:outline-none focus:border-indigo-400 transition-all font-['Poppins']" />
        </motion.div>
                  ...
                  <Link to={`/courses/${course.id}`}>
                    ...
                    <div className="h-[100px] relative overflow-hidden flex items-center justify-between px-[22px]"
                      ...
                      <div className="relative z-[1]">
                        <div className="text-lg font-extrabold text-white font-['Poppins'] tracking-tight" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.15)' }}>{course.title}</div>
                        <div className="text-[11px] text-white/85 font-semibold mt-0.5 font-['Poppins']">{course.subject}</div>
                      </div>
                    ...
                    <div className="p-[18px_22px]">
                      <div className="flex items-center gap-2 mb-3 text-xs text-theme-text-muted font-semibold">
                        <span className="px-2 py-0.5 rounded-md bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-bold text-[11px] font-['Poppins']">{course.chapters} Chapters</span>
                      ...
                      <div className="flex items-center gap-2.5 mb-3">
                        ...
                        <span className="text-xs font-extrabold text-theme-text min-w-[32px] text-right font-['Poppins']">{pct}%</span>
                      </div>
                      <div className="flex gap-2">
                        <button className="flex-1 py-2.5 rounded-xl text-xs font-bold text-white bg-indigo-600 shadow-md shadow-indigo-500/20 hover:bg-indigo-700 transition-all flex items-center justify-center gap-1.5 font-['Poppins']">
                          <BookOpen className="w-3.5 h-3.5" /> Continue
                        </button>
                        <button className="flex-1 py-2.5 rounded-xl text-xs font-bold text-indigo-600 bg-indigo-100 dark:bg-indigo-900/20 hover:bg-indigo-200 dark:hover:bg-indigo-900/30 transition-all flex items-center justify-center gap-1.5 font-['Poppins']">
